...
final class Javascript
{
	...
	/**
	 * The script to add. This can be an url if its an file or a script block.
	 * @var string
	 */
	private $script;

	/**
	 * Flag for external files. External files wont be minified.
	 * @var bool
	 */
	...
	/**
	 * Sets the objects type. Select from "file", "script", "ready", "block" or "var".
	 * @param string $type
	 * @throws Error
	 * @return \Web\Framework\Lib\Javascript
	 */
	...
	public static function getFile($url, $defer=false, $is_external=false)
	{
		...
		return self::factory()->setType('file')
							  ->setScript($url)
	...
	/**
	 * Html5Shim support without autoadding. Returns the javascript object.
	 * @param bool $defer
	 * @return Javascript
	 */
	public static function getHtml5Shim($defer=true)
	{
		return self::factory()
				->setType('block')
				->setDefer($defer)
				->setScript('<!--[if lt IE 9]><script src="http://html5shim.googlecode.com/svn/trunk/html5.js"></script><![endif]-->');
	}
	...
	/**
	 * Modernizr support without auto adding. Returns a javascript object.
	 * @param string $path
	 * @param bool $defer
	 * @return Javascript
	 */
	public static function getModernizr($path, $defer=false)
	{
		...
		return self::factory()
			->setType('file')
			->setDefer($defer)
			->setScript($path . '/modernizr.min.js');
	}
